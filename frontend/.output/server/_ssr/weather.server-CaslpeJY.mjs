//#region node_modules/.nitro/vite/services/ssr/assets/weather.server-CaslpeJY.js
var WEATHER_CODES = {
	0: "Clear sky",
	1: "Mainly clear",
	2: "Partly cloudy",
...
async function fetchFullWeather(lat, lon, forecastDays = 16) {
	...
	const num = (v) => v !== null && v !== void 0 ? Number(v) : null;
	return {
		current: {
			...
			visibility: Number(c.visibility ?? 0) / 1e3,
...
async function fetchHistoricalWeather(lat, lon, startDate, endDate) {
	...
	return (d.time ?? []).map((date, i) => {
		const noonIdx = hourlyTimes.findIndex((t) => t.startsWith(date) && (t.includes("T12") || t.includes("T11")));
		return {
			date,
			...
			soilTemp0cm: noonIdx >= 0 && h.soil_temperature_0cm?.[noonIdx] != null ? Number(h.soil_temperature_0cm[noonIdx]) : null,
			soilMoisture: noonIdx >= 0 && h.soil_moisture_0_to_1cm?.[noonIdx] != null ? Number(h.soil_moisture_0_to_1cm[noonIdx]) : null
		};
	});
}
...
//#endregion
export { fetchFullWeather, fetchHistoricalWeather, fetchWeather, reverseGeocode };
