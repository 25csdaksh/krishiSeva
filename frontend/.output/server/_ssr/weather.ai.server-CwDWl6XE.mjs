...
//#region node_modules/.nitro/vite/services/ssr/assets/weather.ai.server-CwDWl6XE.js
...
function ruleBasedAnalysis(wd) {
	...
	const moisture = soil.soilMoisture3to9cm ?? .2;
	...
	const scoreLabel = score >= 80 ? "Excellent" : score >= 65 ? "Good" : score >= 45 ? "Fair" : score >= 25 ? "Poor" : "Dangerous";
	const diseaseScore = Math.round((humidity - 50) * 2.5 + (rain > 0 ? 20 : 0));
	const floodScore = Math.round(rain * 4 + rainProb * .3);
	const droughtScore = Math.round((.3 - Math.min(moisture, .3)) / .3 * 80);
	...
	return {
		...
		todayRecommendations: [
			...
			moisture < .15 ? "Soil moisture is low — consider irrigation" : "Soil moisture adequate — monitor before next irrigation",
		...
		risks: {
			...
			drought: {
				...
				description: moisture < .15 ? "Soil moisture below threshold — irrigation needed." : "Soil moisture within acceptable range."
			},
			windDamage: {
		...
		waterRequirement: `Reference ET₀ is ${et0.toFixed(1)} mm/day. ${moisture < .15 ? "Soil moisture is low — irrigation recommended." : moisture > .28 ? "Soil is saturated — hold irrigation." : "Soil moisture is adequate."}`,
		irrigationAdvice: moisture < .15 ? `Irrigate ${(et0 * 1.2).toFixed(0)}mm today (ET₀ × 1.2 crop factor).` : moisture > .28 ? "Skip irrigation — risk of waterlogging." : "Irrigation not urgent. Monitor tomorrow.",
		...
		actionPlan: [
			{
				priority: 1,
				action: "Morning crop inspection for disease/pest",
				category: "general",
				timing: "morning",
				isAvoid: false
			},
			...moisture < .15 ? [{
				priority: 1,
				action: `Irrigate crops (ET₀: ${et0.toFixed(1)} mm/day)`,
				category: "irrigation",
				timing: "morning",
				isAvoid: false
			}] : [],
			...windSpeed > 15 ? [{
				priority: 2,
				action: "Postpone pesticide/foliar spraying",
				category: "avoid",
				timing: "today",
				isAvoid: true
			}] : [],
			...rain > 10 ? [{
				priority: 2,
				action: "Avoid fertilizer — runoff risk",
				category: "avoid",
				timing: "today",
				isAvoid: true
			}] : [],
			{
				priority: 3,
				action: "Check field drainage and bunds",
				category: "soil",
				timing: "morning",
				isAvoid: false
			}
...
async function analyzeWeatherForFarming(weatherData, profile) {
	const { current, soil, forecast } = weatherData;
	const crops = profile.primary_crops?.length ? profile.primary_crops.join(", ") : "general crops (cotton, wheat, rice, vegetables)";
...
Respond with ONLY valid JSON (no text before or after):
{
  ...
  "actionPlan": [
	...
	try {
		...
		const parseRisk = (r) => ({
			level: validLevels.includes(String(r?.level)) ? r.level : "low",
			score: Math.max(0, Math.min(100, Number(r?.score) || 0)),
			description: String(r?.description || "")
		});
		return {
			farmingScore: Math.max(0, Math.min(100, Number(parsed.farmingScore) || 50)),
			scoreLabel: validScoreLabels.includes(parsed.scoreLabel) ? parsed.scoreLabel : "Fair",
			summary: String(parsed.summary || ""),
			todayRecommendations: Array.isArray(parsed.todayRecommendations) ? parsed.todayRecommendations.map(String).slice(0, 5) : [],
			suitableActivities: Array.isArray(parsed.suitableActivities) ? parsed.suitableActivities.map(String).slice(0, 6) : [],
			activitiesToAvoid: Array.isArray(parsed.activitiesToAvoid) ? parsed.activitiesToAvoid.map(String).slice(0, 4) : [],
			cropRecommendations: Array.isArray(parsed.cropRecommendations) ? parsed.cropRecommendations.slice(0, 8).map((c) => ({
				crop: String(c.crop || ""),
				...
				priority: [
					...
					"medium",
					"low"
				].includes(String(c.priority)) ? c.priority : "medium"
			})) : [],
			risks: {
			...
			actionPlan: Array.isArray(parsed.actionPlan) ? parsed.actionPlan.slice(0, 10).map((a) => ({
...
async function generateHistoricalSummary(historical, periodLabel, crops) {
...
Give ONLY the 2-3 sentence summary. Be specific and practical.`;
	try {
		return (await callGateway([{
			role: "user",
			content: prompt
		}])).trim() || fallbackHistoricalSummary(totalRain, avgTemp, rainyDays, historical.length);
	} catch {
		return fallbackHistoricalSummary(totalRain, avgTemp, rainyDays, historical.length);
	}
}
function fallbackHistoricalSummary(totalRain, avgTemp, rainyDays, totalDays) {
...
async function generateMonthlyOutlook(forecast16, profile) {
	...
	try {
		...
		return {
			...
			rainfallTrend: [
				...
				"normal",
				"below-normal"
			].includes(parsed.rainfallTrend) ? parsed.rainfallTrend : rainTrend,
			temperatureTrend: [
				"above-normal",
				"normal",
				"below-normal"
			].includes(parsed.temperatureTrend) ? parsed.temperatureTrend : tempTrend,
		...
		return {
			...
			suggestedActivities: [
				"Monitor crops weekly for disease/pest",
				"Plan field operations around rain-free windows",
...
//#endregion
export { analyzeWeatherForFarming, generateHistoricalSummary, generateMonthlyOutlook };
